...
/**
 * Interface to perform the Firebase Function calls and expose the results with {@link
 * #getSubscriptions}.
 * <p>
 * Use this class by observing the {@link #getSubscriptions} LiveData. Any server updates will be
 * communicated through this LiveData.
 */
public interface ServerFunctions {
  ...
  /**
   * Fetch basic content and post results to {@link #getBasicContent()}. This will fail if the user
   * does not have a basic subscription.
   */
  void updateBasicContent();

  /**
   * Fetch premium content and post results to {@link #getPremiumContent()}. This will fail if the
   * user does not have a premium subscription.
   */
  void updatePremiumContent();

  /**
   * Fetches subscription data from the server and posts successful results to {@link
   * #getSubscriptions}.
   */
  ...
  /**
   * Transfer subscription to this account posts successful results to {@link #getSubscriptions}.
   */

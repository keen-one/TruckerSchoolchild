...
package softnecessary.furgonok.compras.data;
...
public class DataRepository {
   ...
   * <p>
   * The mediator observes multiple sources. The database source is immediately exposed. The network
   * source is stored in the database, which will eventually be exposed. The mediator provides an
   * easy way for us to use LiveData for both the local data source and the network data source,
   * without implementing a new callback interface.
   */
  ...
  /**
   * TODO(122273956) - Simplify / improve merge algorithm Merge the previous subscriptions and new
   * subscriptions by looking at on-device purchases.
   * <p>
   * We want to return the list of new subscriptions, possibly with some modifications based on old
   * subscriptions and the on-devices purchases from Google Play Billing. Old subscriptions should
   * be retained if they are owned by someone else (subAlreadyOwned) and the purchase token for the
   * subscription is still on this device.
   */
  ...
  /**
   * Modify the subscriptions isLocalPurchase field based on the list of local purchases. Return
   * true if any of the values changed.
   */

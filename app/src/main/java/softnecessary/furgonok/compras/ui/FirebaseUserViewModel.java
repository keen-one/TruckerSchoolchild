...
public class FirebaseUserViewModel extends ViewModel {
  ...
  /**
   * When the user changes, the app needs to notify the server. A normal LiveData is called more
   * often in order to keep the UI up-to-date. This event is a more efficient indicator that the
   * user might have changed. This event could be fired more often than needed because the ViewModel
   * can be destroyed and recreated even if the user does not change. To improve this, we could
   * store authentication information to disk in order to make sure we only call the server when the
   * user actually changes, however this implementation seems to be efficient enough for this
   * sample.
   */

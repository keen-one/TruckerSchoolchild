...
public class BillingUtilities {
   ...
   */
  private static Purchase getPurchaseForSku(@Nullable List<Purchase> purchases, String sku) {
    if (purchases != null) {
  ...
  /**
   * This will return true if the server has a record for the subscription. Sometimes this will
   * return true even if the Google Play Billing APIs return false.
   * <p>
   ...
   * <p>
   * Example: App user buys the subscription on a different device with a different Google account.
   * The server will show that this app user has the subscription, even if the Google account on
   * this device has not purchased the subscription. In this example, the method will return true,
   * even though [deviceHasGooglePlaySubscription] will return false.
   * <p>
   * Example: The app user changes by signing out and signing into the app with a different email
   * address. The server will show that this app user does not have the subscription, by returning
   * an API response indicating that it is ALREADY_OWNED. even if the Google account on this device
   * has purchased the subscription. In this example, the method will return true. This is the same
   * as the result from [deviceHasGooglePlaySubscription].
   */
